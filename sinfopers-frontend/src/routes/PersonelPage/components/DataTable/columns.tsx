// src/routes/PersonelPage/components/DataTable/columns.tsx
...
// Buat function untuk membuat columns berdasarkan role
export const getColumns = (): ColumnDef<Personnel>[] => {
  const { hasRole } = useAuth();
  const canEdit = hasRole([UserRole.ADMIN, UserRole.HR]);

  const baseColumns: ColumnDef<Personnel>[] = [
    {
      id: "number",
      header: "No.",
      cell: ({ row }) => {
...
// Untuk backward compatibility, masih menyediakan array columns statis
