...
const NodeMenuDialog = ({ chartId, item, parentOffsetId }: Props) => {
  ...
  const [errorMessage, setErrorMessage] = useState<string>("Gagal menyimpan data divisi");
  ...
  // Destructure the functions from their respective hooks.
  const { createNode } = usePostCreateNode();
  const { editNode } = usePutEditNode();
      ...
      parentOffsetId: parentOffsetId,
        ...
        .catch((error) => {
          if (error?.response?.data?.message?.includes("already exists")) {
            setErrorMessage("Anggota ini sudah terdaftar dalam struktur divisi");
          } else {
            setErrorMessage("Gagal menyimpan data divisi");
          }
  ...
  return (
      <DialogContent onCloseAutoFocus={handleClose}>
        {dialogState === DialogState.success && (
            <SuccessDialog message="Data divisi berhasil disimpan" />
        )}
